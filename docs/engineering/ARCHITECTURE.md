...
│
├── core-ai/               -- Phase 3: LLM/임베딩 포트 인터페이스
...
│   ├── gemini/            -- Google Gemini API 어댑터
│   ├── openai/            -- OpenAI API 어댑터
│   ├── ollama/            -- Ollama 로컬 LLM 어댑터
│   └── config/            -- LLM 설정 및 모델 선택
│
...
│   ├── libs/
│   └── requirements.txt
├── deploy/                  # Phase 2: Helm Charts
│   ├── charts/issuehub/
│   └── scripts/
├── docker-compose.yml       # Phase 1 로컬 개발 (유지)
...
| 단계 | 오케스트레이션 | 외부 DB (Managed) | 로컬 서비스 (컨테이너) |
|------|--------------|-------------------|---------------------|
| **Phase 1** | Docker Compose | Aiven PostgreSQL + Upstash Redis | OpenSearch |
| **Phase 2+** | k3d 클러스터 + Helm Charts | Aiven PostgreSQL + Upstash Redis | OpenSearch |
| **Phase 3+** | k3d 클러스터 + Helm Charts | Aiven PostgreSQL + Upstash Redis | OpenSearch, Airflow (K8sExecutor) |

> **참고**: PostgreSQL과 Redis는 외부 매니지드 서비스(Aiven, Upstash)를 사용하며 로컬에 띄우지 않는다. OpenSearch와 Airflow는 로컬 Docker/k3d 클러스터에서 운영한다.
...
| **검색** | OpenSearch (nori 플러그인) | 한국어 형태소 분석 포함 |
