...
| `embedding` | vector(1536) | NULLABLE | 이슈 제목+설명 임베딩 벡터 |
...
| `embedding` | vector(1536) | NULLABLE | 정책 내용 임베딩 벡터 |
...
이슈와 정책의 텍스트 내용을 OpenAI `text-embedding-3-small` (1536 차원) 모델로 임베딩하여 저장한다.
...
| 트리거 | 동작 |
|--------|------|
| 이슈 생성 | Kafka 이벤트 → 비동기 임베딩 생성 |
| 이슈 제목/설명 수정 | Kafka 이벤트 → 임베딩 재생성 |
