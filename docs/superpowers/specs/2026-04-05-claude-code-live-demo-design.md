...
## 핵심 프레임워크: 하네스 엔지니어링 4기둥

발표 전체를 관통하는 뼈대. 각 Part가 하나의 기둥에 대응:

| 하네스 기둥 | 비유 | Claude Code 구현 | 발표 파트 |
|------------|------|-----------------|----------|
| **기둥 1: 컨텍스트 파일** | AI에게 주는 업무 매뉴얼 | CLAUDE.md + Skills | Part 2 |
| **기둥 2: CI/CD 게이트** | 규칙을 시스템이 강제 | Hooks (100% 실행 보장) | Part 3 |
| **기둥 3: 도구 경계** | 쓰는 AI와 검토하는 AI 분리 | SubAgent + Expert Panel | Part 4 |
| **기둥 4: 피드백 루프** | 테스트 통과까지 자동 수정 | /dev-cycle (계획→실행→검증→수정) | Part 5 |
...
2. 하네스 엔지니어링 프레임워크 소개 (2분)
   ...
   > "프롬프트 엔지니어링은 '부탁'입니다. 하네스 엔지니어링은 '강제'입니다.
...
2. **라이브로 새 Skill 만들기** (2분):
   - `.claude/skills/jira-implement/SKILL.md` 작성
   - frontmatter (name, description) + `$ARGUMENTS` + 절차 보여주기
...
```
코드 작성 후 린트 꼭 돌려줘.
커밋 메시지에 Jira 티켓 번호 넣어줘.
```

→ **"AI가 가끔 빼먹습니다. 프롬프트는 '부탁'이니까요."**
...
```
이 코드 리뷰해줘. 보안, 성능, 가독성 다 체크하고...
```

→ **"한 명이 모든 관점에서 대충 리뷰합니다. 리뷰 결과가 메인 대화에 쌓여서 컨텍스트가 오염되고요."**
...
4. **라이브 SubAgent 생성** (1분):
   - `/agents` → Create new agent → 간단한 에이전트 하나 만들기
   - "이렇게 만들면 즉시 `@에이전트명`으로 호출 가능"
...
**하네스 4기둥 = Claude Code 4도구**:

| 마구 부품 | Claude Code | 핵심 |
|----------|-------------|------|
| 컨텍스트 파일 | **CLAUDE.md + Skill** | AI에게 규칙과 절차를 주는 매뉴얼 |
| CI/CD 게이트 | **Hook** | 프롬프트는 부탁, Hook은 강제 |
| 도구 경계 | **SubAgent + Expert Panel** | 쓰는 AI와 검토하는 AI를 분리 |
| 피드백 루프 | **/dev-cycle** | 검증 실패 → 자동 수정 루프 |
