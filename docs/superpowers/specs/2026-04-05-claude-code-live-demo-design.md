...
1. Claude Code에 간단한 요청:
   ```
   frontend/src/features/projects/components/hello.tsx에 간단한 컴포넌트 하나 만들어줘
   ```
2. 파일이 생성되지만 **린트가 돌지 않음** — 세미콜론 누락, import 순서 등 그대로
3. 화면에 보여주며:
   → **"프롬프트에 '린트 돌려줘'라고 썼어도 AI가 빼먹을 수 있습니다. 프롬프트는 '부탁'이니까요."**
...
> **전환 멘트**: "지금까지 Guide(Skill, SubAgent)와 Sensor(Hook)를 각각 보여드렸습니다.
