CREATE TABLE pending_issues (
    ...
    CONSTRAINT uq_pending_issues_source_message UNIQUE (source_message_id)
);
...
COMMENT ON COLUMN pending_issues.source_message_id IS '멱등성용 원본 메시지 ID (Slack ts 등) — UNIQUE 제약으로 중복 방지';
